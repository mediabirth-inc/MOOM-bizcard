...
export default function EditPartnerPage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      setForm({ name: partner.name, furigana: partner.furigana, email: partner.email, phone: partner.phone });
      setFetching(false);
    }
    load();
  }, [id]);
  ...
  return (
    <div className="min-h-screen flex" style={{ background: '#F0EFED' }}>
      ...
      <div className="flex-1" style={{ marginLeft: 240 }}>
        ...
        >
          <div>
            <h1 className="text-xl font-light" style={{ color: '#1A1A1A' }}>パートナー情報編集</h1>
            <p className="text-xs mt-0.5" style={{ color: '#ABABAB' }}>変更を保存すると電子名刺に即時反映されます</p>
          </div>
          <Link
            href="/admin"
        ...
        <main className="px-10 py-10">
          {fetching ? (
            <div className="text-sm" style={{ color: '#ABABAB' }}>読み込み中...</div>
          ) : (
              ...
              <form onSubmit={handleSubmit} className="space-y-5">
                ...
                <div className="pt-2 flex gap-3">
                  <button
                    type="submit"
                    disabled={loading}
                    className="flex-1 text-white text-sm font-medium py-3.5 rounded-full transition-opacity hover:opacity-80 disabled:opacity-50"
                  ...
                  <Link
                    href="/admin"
                    className="flex items-center justify-center text-sm font-medium py-3.5 px-6 rounded-full border transition-colors hover:bg-gray-50"
                    style={{ borderColor: '#E5E4E0', color: '#ABABAB' }}
                  >
                    キャンセル
                  </Link>
                </div>
